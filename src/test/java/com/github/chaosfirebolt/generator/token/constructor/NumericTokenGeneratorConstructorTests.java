 ...
 */
public class NumericTokenGeneratorConstructorTests extends TokenGeneratorConstructorTests {

    @Override
    protected List<InvalidConstructorInvocationWrapper> getLengthParamNegative() {
        int length = -6;
        return Collections.singletonList(buildWrapperForIllegalArgument(() -> new NumericTokenGenerator(length), length));
    }
    ...
    protected List<Callable<? extends TokenGenerator<?>>> getValidLengthParams() {
        return Collections.singletonList(() -> new NumericTokenGenerator(9));
    }

    @Override
    protected List<InvalidConstructorInvocationWrapper> getParamsDoNotConformRules() {
        Executable executable = () -> new NumericTokenGenerator(9, Collections.singletonList(MIN_TOKEN_LENGTH_VALIDATOR));
    ...
    protected List<Callable<? extends TokenGenerator<?>>> getParamsConformRules() {
        return Collections.singletonList(() -> new NumericTokenGenerator(30, Collections.singletonList(MIN_TOKEN_LENGTH_VALIDATOR)));
    }

    @Override
    protected List<InvalidConstructorInvocationWrapper> getParamsWithRandomDoNotConformRules() {
        Executable executable = () -> new NumericTokenGenerator(new Random(), 9, Collections.singletonList(MIN_TOKEN_LENGTH_VALIDATOR));
        return Collections.singletonList(buildWrapperForInvalidGeneratorRule(executable));
    }

    @Override
    protected List<Callable<? extends TokenGenerator<?>>> getParamsWithRandomConformRules() {
        return Collections.singletonList(() -> new NumericTokenGenerator(new Random(), 40, Collections.singletonList(MIN_TOKEN_LENGTH_VALIDATOR)));
    }
}